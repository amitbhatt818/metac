...
type ConfigMetaController struct {
	...
	WaitTimeoutForStart time.Duration

	// Interval between retries to start all watch controllers
	WaitIntervalBetweenStarts time.Duration
...
func SetMetacConfigToRetryIndefinitelyForStart(enabled *bool) ConfigMetaControllerOption {
	return func(c *ConfigMetaController) error {
		...
		c.WaitIntervalBetweenStarts = 1 * time.Minute
		c.RetryIndefinitelyForStart = k8s.BoolPtr(true)
		return nil
	}
}
...
func NewConfigMetaController(
	...
	ctl := &ConfigMetaController{
		...
		WaitTimeoutForStart: 30 * time.Minute,
		// - Interval between retries is 1 second
		WaitIntervalBetweenStarts: 1 * time.Second,
		opts:                      opts,
...
func (mc *ConfigMetaController) startWithRetries(
	...
	for {
		...
		if time.Since(start) > mc.WaitTimeoutForStart &&
			(mc.RetryIndefinitelyForStart == nil ||
				!*mc.RetryIndefinitelyForStart) {
			{
				return errors.Errorf(
					"Condition timed out after %s: %+v: %s",
					mc.WaitTimeoutForStart,
					err,
					mc,
				)
			}
		...
		time.Sleep(mc.WaitIntervalBetweenStarts)
	}
}
